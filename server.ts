...
const AUDIT_LOGS_FILE = process.env.NETLIFY 
  ? "/tmp/audit_logs.json" 
...
const PHOTOS_DIR = process.env.NETLIFY 
  ? "/tmp/astro_photos" 
...
function savePhotoToDisk(id: string, photoBase64: string) {
  ...
  try {
    ...
    fs.writeFileSync(filePath, photoBase64, "utf8");
  } catch (err) {
    console.warn(`Could not save photo backup to disk for ID ${id}:`, err);
  }
}
...
function getPhotoFromDisk(id: string): string | null {
  ...
  try {
    ...
    console.warn(`Could not read photo from disk for ID ${id}:`, err);
  }
  return null;
}
...
async function syncReportsFromFirestore(): Promise<void> {
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      if (docId !== "google_drive_tokens" && docId !== "usage_logs") {
        if (deletedReportIds.has(docId)) {
          // If this was deleted, ensure it's deleted from Firestore as well
    ...
    // Ensure any locally created reports not yet in Firestore get uploaded to Firestore asynchronously
    for (const [rId, rep] of cachedReportsMap.entries()) {
      if (rId && !firestoreIds.has(rId) && !deletedReportIds.has(rId) && rep && rep.id && rId !== "google_drive_tokens" && rId !== "usage_logs") {
        setDoc(doc(firestoreDb, "reports", rId), prepareReportForFirestore(rep), { merge: true }).catch(() => {});
      }
    }
  } catch (err: any) {
    recordFirestoreFailure(err);
...
async function getReportsAsync(forceFreshSync: boolean = false): Promise<any[]> {
  ...
  // 2. Sync from Firestore in background or fast sync without blocking if data exists
  if (isFirestoreAvailable()) {
    const isCacheEmpty = cachedReportsMap.size === 0;
    const shouldSync = forceFreshSync || (Date.now() - lastFirestoreSyncTime > 15000) || isCacheEmpty;

    if (shouldSync) {
      if (isCacheEmpty) {
        // If memory is completely empty, wait briefly (max 2s) for initial hydration
        try {
          await withTimeout(syncReportsFromFirestore(), 2000);
        } catch (e) {}
      } else {
        // Run sync in the background so HTTP response is returned immediately (0ms)
...
function cleanReportForAdminList(report: any): any {
  ...
  // Extract lean, essential fields for the admin list view without heavy predictions/calculations
  const clean: any = {
    id: reportIdStr,
    ...
    comment: typeof report.comment === "string" ? report.comment.slice(0, 300) : (typeof report.feedback === "string" ? report.feedback.slice(0, 300) : ""),
    feedback: typeof report.feedback === "string" ? report.feedback.slice(0, 300) : (typeof report.comment === "string" ? report.comment.slice(0, 300) : ""),
  ...
  // Include clean full chat history if present
  if (Array.isArray(report.chatHistory) && report.chatHistory.length > 0) {
    clean.chatHistory = report.chatHistory.map((m: any) => ({
...
app.get("/api/admin/reports", requireAdminAuth, async (req, res) => {
  try {
    ...
    const customLimit = typeof req.query.limit === "string" && !isNaN(Number(req.query.limit)) ? Number(req.query.limit) : null;
    
    let finalReports: any[];
    if (fetchAll) {
      finalReports = workingReports;
    } else if (customLimit && customLimit > 0) {
      finalReports = workingReports.slice(0, customLimit);
    } else if (hasDateFilter) {
      // For date search, return all matching records in that range by default (or up to 500)
      finalReports = workingReports.slice(0, 500);
    } else {
      // Default initial view: exactly the latest 100 records
      finalReports = workingReports.slice(0, 100);
    }
    ...
    res.json({ 
      ...
      isShowingAll: fetchAll || finalReports.length >= workingReports.length,
...
app.get("/api/admin/reports/:id/photo", requireAdminAuth, async (req, res) => {
  try {
    ...
    const photo = report.palmImageBase64 || 
                  report.dehalakshanaImageBase64 || 
                  report.storedImage || 
                  report.imageBase64 || 
                  report.image || 
    ...
    if (!photo) {
      return res.status(404).json({ success: false, error: "ඡායාරූපයක් සුරැකී නොමැත (No photo attached)." });
    }
...
app.get("/api/admin/security-audit-logs", requireAdminAuth, async (req, res) => {
  try {
    ...
    if (isFirestoreAvailable()) {
      try {
        const snap = await withTimeout(getDocs(collection(firestoreDb, "security_audit_logs")), 2000);
        const firestoreLogs: any[] = [];
        snap.forEach((d) => {
          firestoreLogs.push(d.data());
        });
        if (firestoreLogs.length > 0) {
          const map = new Map<string, any>();
          [...firestoreLogs, ...logs].forEach((l) => {
            if (l && l.id) map.set(l.id, l);
          });
          logs = Array.from(map.values());
          logs.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
          cachedAuditLogs = logs.slice(0, 500);
          saveAuditLogsToDisk(cachedAuditLogs);
        }
      } catch (e) {}
    }
